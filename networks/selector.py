...
class Selector(nn.Module):
	def __init__(self, config, relation_dim):
		...
		self.relation_matrix = nn.Embedding(self.config.num_classes, 1*relation_dim+0*self.config.data_word_vec.shape[1])
		...
		self.attention_matrix = nn.Embedding(self.config.num_classes, 1*relation_dim+0*self.config.data_word_vec.shape[1])
		self.init_weights()
		self.scope = None
