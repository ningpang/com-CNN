...
class Selector(nn.Module):
	def __init__(self, config, s_relation_dim):
		super(Selector, self).__init__()
		self.config = config
		# self.relation_matrix = nn.Embedding(self.config.num_classes, relation_dim+self.config.data_word_vec.shape[1])
		self.relation_matrix = nn.Embedding(self.config.num_classes, s_relation_dim+2*self.config.data_word_vec.shape[1])
		...
		self.attention_matrix = nn.Embedding(self.config.num_classes, s_relation_dim+2*self.config.data_word_vec.shape[1])
		self.init_weights()
		self.scope = None
