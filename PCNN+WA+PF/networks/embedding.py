...
class Embedding(nn.Module):
	...
	def forward(self):
		...
		sen_embedding = torch.cat((word, pos1, pos2), dim = 2) # n*length*dim
		WA = self.entity_attention(word, tail-head)


		return sen_embedding, WA, tail-head

	def entity_attention(self, sentence, entity):
		ent = entity.unsqueeze(1).expand(-1, self.config.max_length, -1)
		sen_ent = torch.cat([sentence, ent], 2)
		att_score = F.softmax(self.attention(sen_ent), 1)
		wa = (att_score*sentence).sum(1)
		return wa
