...
class _GRU(nn.Module):
	def __init__(self, config):
		...
		self.config = config
		self.in_width = self.config.word_size +self.config.tag_size + self.config.deprel_vec.shape[1]+self.config.dir_size
		self.out_channels = self.config.path_hidden_size
		self.gru = nn.LSTM(self.in_width, self.out_channels, bidirectional=True)
	def forward(self, embedding):
...
class GRU(nn.Module):
	...
	def forward(self, embedding):
		...
		x_f = x[:, :, :self.config.path_hidden_size]
		x_b = x[:, :, self.config.path_hidden_size:]
		x = x_f + x_b
		return x
